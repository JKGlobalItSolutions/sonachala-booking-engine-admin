...
const GuestDetails = () => {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'Homestays', uid, 'Guest Details'));
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      setGuests(prev => {
        if (prev.length > 0 && guestsData.length > prev.length) {
          toast.info(`New homestay booking received!`);
        }
        return guestsData;
      });
    });
  ...
  const categorizeGuests = (guests) => {
    ...
    guests.forEach(guest => {
      const checkoutDate = guest.checkOutRaw;
  ...
  const sortedGuests = (guests) => {
    return guests.sort((a, b) => b.checkInRaw - a.checkInRaw);
  };
    ...
    return (
      <div className="card mb-4" key={guest.id}>
        <div className="card-body">
          ...
          <h3 className="mb-4">
            {guest.fullName || 'Guest Name Not Available'}
          ...
          <div className="row g-4">
            <div className="col-md-6">
              ...
              <div className="mb-3">
                ...
                <div>{guest.phoneNumber || 'Phone Not Available'}</div>
              </div>
              <div className="mb-3">
