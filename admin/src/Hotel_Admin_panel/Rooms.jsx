...
const RoomDetails = () => {
  ...
  const confirmRemoveRoom = async () => {
    if (roomToDelete) {
      const homestayId = auth.currentUser.uid;
      try {
        await deleteDoc(doc(db, 'Hotels', homestayId, 'Rooms', roomToDelete));
  ...
  const calculateAndStoreMaxGuests = async () => {
    ...
    try {
      await updateDoc(doc(db, 'Hotels', homestayId), { totalMaxGuests: totalMaxGuests });
    } catch (error) {
...
`}</style>
      ...
      {rooms.length === 0 ? (
        <div className="text-center py-5 bg-white rounded-3 shadow-sm p-5">
          ...
          <div className="add-room-button-container mb-4">
            <Button style={{ backgroundColor: '#038A5E', borderColor: '#038A5E', color: '#fff', padding: '10px 30px' }} onClick={handleAdd}>
              + Add Your First Room
            </Button>
          </div>
        ...
        <>
            ...
            <div key={room.id} className="room-card">
              ...
              <div className="card-footer">
                <div className="action-buttons">
                  <Button style={{ backgroundColor: '#038A5E', borderColor: '#038A5E', color: '#fff' }} onClick={() => handleEdit(room)}>Edit</Button>
          ...
          <div className="add-room-button-container">
            <Button style={{ backgroundColor: '#038A5E', borderColor: '#038A5E', color: '#fff' }} onClick={handleAdd}>+ Add New Room</Button>
          </div>
        </>
      )}
        ...
        <Modal.Body>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" name="customName" placeholder="e.g. Ocean View Suite" value={currentRoom?.customName || ''} onChange={handleInputChange} />
            ...
            <Button style={{ backgroundColor: '#038A5E', borderColor: '#038A5E', color: '#fff' }} type="submit" disabled={isUploading}>
      ...
      >
        ...
        <Modal.Footer>
          ...
          <Button
            style={{ backgroundColor: '#038A5E', borderColor: '#038A5E', color: '#fff' }}
